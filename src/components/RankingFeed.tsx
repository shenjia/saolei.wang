...
export function RankingFeed(props: RankingFeedProps) {
  ...
  function scrollToMe() {
    const el = document.getElementById(`id_${myUid}`);
    if (el) {
  ...
  // 「我在哪里」：① 已加载 → 滚动；② 30 页内 → 并发拉取后滚动；③ 更远 → 跳 whereami
  async function whereAmI() {
    if (!myUid || locating) return;
    if (scrollToMe()) return;
    const targetPage = Math.ceil(((rowRankById(rows, myUid) as number | undefined) ?? 0) / pageSize);
    ...
    try {
      ...
      requestAnimationFrame(() => {
        if (!scrollToMe()) setHint("未找到你的位置，请稍后重试");
        else setHint("");
      });
    } catch {
      jumpToWhereami();
  ...
  function jumpToWhereami() {
    const qs = new URLSearchParams({ id: String(myUid), by });
    if (params.view) qs.set("view", String(params.view));
